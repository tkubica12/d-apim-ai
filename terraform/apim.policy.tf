...
resource "azapi_resource" "openai_api_policy" {
    ...
    body = {
        properties = {
            format = "rawxml"
            value  = local.genai_policy
        }
    }
}

resource "azapi_resource" "product_silver_policy" {
    ...
    body = {
        properties = {
            format = "rawxml"
            value  = local.silver_policy
        }
    }
}

resource "azapi_resource" "product_caching_policy" {
    type      = "Microsoft.ApiManagement/service/products/policies@2024-06-01-preview"
    name      = "policy"
